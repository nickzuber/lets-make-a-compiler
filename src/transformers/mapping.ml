...
let build_liveness_mapping (instructions : Select.instruction list) : (Select.instruction, Select.arg list) Hashtbl.t =
  let variable_size = List.length instructions in
  let liveness = Hashtbl.create variable_size in
  let assign instrs : unit =
    match instrs with
    | [] -> ()
    | instr :: rest -> Hashtbl.add liveness instr []
  in assign instructions;
  liveness
